...
async function openPage(browser: Browser): Promise<Page> {
  ...
  await page.waitForFunction(() => (window as any).__spiderMonkeyTestReady === true, {}, { timeout: 180_000 });
  return page;
}
...
async function main() {
  if (!existsSync(VFS_IMAGE) || process.env.SPIDERMONKEY_OFFICIAL_REBUILD_VFS === "1") {
    await new Promise<void>((resolveDone, reject) => {
      const proc = spawn("bash", [join(REPO_ROOT, "images/vfs/scripts/build-spidermonkey-test-vfs-image.sh")], {
        cwd: REPO_ROOT,
        stdio: "inherit",
        env: { ...process.env },
      });
