...
describe("centralized select/pselect timeout retries", () => {
  ...
  it("preserves a finite host-side epoll_wait deadline across retries", () => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
    ...
    worker.dequeueSignalForDelivery = vi.fn(() => 0);
    worker.completeChannelRaw = vi.fn();
    worker.relistenChannel = vi.fn();
    ...
    worker.handleEpollPwait(channel, ABI_SYSCALLS.EpollWait, [7, 4096, 1, 10, 0, 0]);
  ...
  it("interrupts host-parked blocking accept when a handler signal is pending", () => {
    const kernelMemory = createSharedMemory();
    const processMemory = createSharedMemory();
    const scratchOffset = 128;
    const handleChannel = vi.fn(() => {
    ...
    worker.kernelMemory = kernelMemory;
    worker.scratchOffset = scratchOffset;
    worker.dequeueSignalForDelivery = vi.fn(() => 3);
    ...
    const origArgs = [10, 1024, 2048, 0, 0, 0];
    const processView = new DataView(processMemory.buffer);
    processView.setUint32(CH_SYSCALL, ABI_SYSCALLS.Accept, true);
    for (let i = 0; i < origArgs.length; i++) {
      processView.setBigInt64(CH_ARGS + i * CH_ARG_SIZE, BigInt(origArgs[i]), true);
    }
