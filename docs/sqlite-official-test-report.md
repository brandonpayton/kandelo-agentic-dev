...
## 2026-06-05 PR Resume Status

This PR carries forward the reference branch work into a focused harness and
kernel/host update.  The new reusable entry point is:

```bash
bash scripts/run-sqlite-project-unit-tests.sh \
  --host both \
  --permutation full \
  --jobs 2 \
  --timeout-ms 21600000
...
It runs the same upstream SQLite `test/testrunner.tcl` permutation through
Kandelo's Node and browser hosts, writes per-host artifacts, and emits a
combined markdown summary at `<results-root>/combined-summary.md`.

Current validation on this branch:

| Check | Result |
|-------|--------|
| Node `func7.test` through new harness | 1/1 jobs passed, 73 SQLite cases, 0 errors (`test-runs/harness-smoke-node-newfork`) |
| Browser `func7.test` through new harness | 1/1 jobs passed, 73 SQLite cases, 0 errors (`test-runs/harness-smoke-browser-newfork`) |
| `bash scripts/check-abi-version.sh` | Passed; ABI snapshot/header/TS bindings are in sync with ABI_VERSION 14 |
| `npm --prefix host run typecheck` | Passed |
| `cargo test --target <host-triple> -p kandelo --lib` | Passed: 872/872 kernel unit tests |
| `cargo test --target <host-triple> -p wasm-posix-shared` | Passed: 8/8 shared ABI/layout tests |
| `cargo test --target <host-triple> -p fork-instrument` | Passed |
| Focused host regression set | Passed: 61 tests, 2 skipped across mmap/shared-fs, process reap/wait, pthread slot reuse, select retry, lock table, and VFS tests |

Current SQLite host counts/challenges:

| Host | Current branch checkpoint | Challenge |
|------|---------------------------|-----------|
| Node | Smoke passed: 1/1 jobs, 73/73 cases. Reference full run from 2026-06-04 remains 1416/1416 jobs and 1,703,255 cases. | A local `sort4.test` rerun on 2026-06-05 did not finish inside a 300s outer timeout while the shared runner machine was under heavy cross-session load (load average ~15). Treat as an unresolved local capacity/checkpoint issue until rerun on an idle runner; do not xfail or skip it. |
| Browser | Smoke passed: 1/1 jobs, 73/73 cases. Reference full browser checkpoint remains 40/1416 jobs, 12,206 cases, 0 errors, timed out/stalled in `test/sort4.test`. | Full browser still needs `sort4.test` diagnosis and an end-to-end rerun. Local browser execution also requires Playwright's Chromium shared-library dependencies and a built `rootfs.vfs`; the SQLite-only Vite input path now avoids unrelated demo binary scans. |

Additional changes made while resuming:

- Added a future-facing both-host wrapper (`scripts/run-sqlite-project-unit-tests.sh`)
  that preserves upstream SQLite testrunner artifacts and summarizes pass counts.
- Bumped the ABI to 14 because the guest-visible `struct stat` layout changed
  (`st_rdev`, `st_blksize`, and `st_blocks` are now marshalled).
- Rebuilt/revalidated guest SQLite artifacts with the updated fork
  instrumentation from the reference branch.
- Fixed the standalone MAP_SHARED regression fixture so it verifies POSIX
  shared mapping coherence after `munmap()` and exercises `mremap()`.
