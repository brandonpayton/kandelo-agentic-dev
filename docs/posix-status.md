...
| `connect()` | Partial | AF_UNIX SOCK_DGRAM connects to a bit-bucket pattern. AF_INET TCP is host-backed and works over Node external TCP or the browser local virtual-network backend. AF_INET UDP connect stores the peer, auto-binds an ephemeral local port when needed, filters receives to the connected peer, and supports AF_UNSPEC unconnect. UDP loopback and local virtual routes are supported; browser demos may opt into a configured WebRTC DataChannel UDP relay, otherwise external raw UDP routes return ENETUNREACH without a HostIO UDP proxy/backend. |
...
| `sendto()` / `recvfrom()` | Partial | AF_INET UDP loopback and local virtual-network send/receive, connected and unconnected sendto, source address reporting, and connected receive filtering are implemented. External raw UDP routes return ENETUNREACH unless a host UDP proxy/backend such as the browser WebRTC relay is configured; raw external routes remain narrowly xfailed in the Sortix UDP suite. |
...
| **External raw UDP routes** | socket | AF_INET SOCK_DGRAM has POSIX-style in-kernel loopback/virtual semantics, but browsers cannot expose raw UDP directly and Node raw UDP is not yet wired behind HostIO. Browser demos can opt into a WebRTC DataChannel UDP relay for a configured peer; otherwise non-loopback UDP routes return ENETUNREACH. |
